...
public:
    ...
    static std::shared_ptr<GossipDatabase<StoredDataType>> get_instance(int database_size, int number_of_message, int send_tag, MPI_Comm _world){
        static std::unordered_map<int, std::shared_ptr<GossipDatabase<StoredDataType> > > instances;
        if(instances.find(send_tag) == instances.end()) {
            instances[send_tag] = std::shared_ptr<GossipDatabase<StoredDataType>>(new GossipDatabase(database_size, number_of_message, send_tag, 0, _world));
        }
        return instances[send_tag];
    }
    ...
    StoredDataType variance() const {
        const auto mu = mean();
        const auto data = get_all_meaningfull_data();
