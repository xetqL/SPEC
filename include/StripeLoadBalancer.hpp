...
private:
        ...
        if(N <= worldsize / 2){
            if(!myrank) {
                ...
                for(PEIndex p = 0; p < (unsigned int) worldsize; ++p) {
                    ...
                    while(((current_process_workload + rows_load[end_stripe] <= desired_workload) || p == (unsigned int) worldsize-1)  && end_stripe < (unsigned int) sizeY) {
                    ...
                    assert(begin_stripe < (unsigned int) sizeY);
    ...
    void resolve_process_membership(unsigned int gid, PEIndex* p){
        ...
        for((*p) = 0; (*p) < (unsigned int) worldsize; ++(*p)) {
            auto b = partition[2 * (*p)];
            auto e = partition[2 * (*p) + 1];
            if(b <= (unsigned int) row && (unsigned int) row <= e) return;
        }
