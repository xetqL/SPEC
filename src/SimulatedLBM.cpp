...
void SimulatedLBM::run(float alpha) {
    ...
    for(unsigned int step = 0; step < MAX_STEP; ++step) {
        ...
        STOP_TIMING(main_loop);
        std::cout << rank << "  " << main_loop << std::endl;
    ...
    MPI_Allreduce(MPI_IN_PLACE, &total_gossip_time, 1, MPI_DOUBLE, MPI_MAX, world);
